...
public class QuarkusLLMAdapter implements LLMFunction {
    ...
    public CompletableFuture<String> apply(
        ...
        // Execute with proper classloader context
        final ClassLoader originalClassLoader = Thread.currentThread().getContextClassLoader();
        Thread.currentThread().setContextClassLoader(QUARKUS_CLASSLOADER);
        
        try {
            return CompletableFuture.supplyAsync(() -> {
                // Re-set classloader in the async thread
                final ClassLoader asyncOriginalClassLoader = Thread.currentThread().getContextClassLoader();
                Thread.currentThread().setContextClassLoader(QUARKUS_CLASSLOADER);
                
                try {
                    final int historySize = historyMessages != null ? historyMessages.size() : 0;
                    LOG.debugf("LightRAG LLM request - prompt length: %d, system prompt: %s, history size: %d, thread: %s, classloader: %s",
                    ...
                    Thread.currentThread().setContextClassLoader(asyncOriginalClassLoader);
                }
            });
        } finally {
            // Restore original classloader
