 ...
 * 
 * <p>
 * Uses Large Language Model to produce coherent summaries when accumulated
 * entity descriptions exceed the configured token threshold. Results are cached
 * in the ExtractionCache to avoid redundant LLM calls.
 * </p>
 * 
...
public class LLMDescriptionSummarizer implements DescriptionSummarizer {
    ...
    /**
     * Configurable system prompt for summarization (injected from
     * application.properties).
     */
    @Inject
    @ConfigProperty(name = "lightrag.summarization.system.prompt", defaultValue = DEFAULT_SUMMARIZATION_SYSTEM_PROMPT)
    String systemPrompt;

    /**
     * Configurable user prompt template for summarization (injected from
     * application.properties).
     */
    @Inject
    @ConfigProperty(name = "lightrag.summarization.user.prompt", defaultValue = DEFAULT_SUMMARIZATION_USER_PROMPT_TEMPLATE)
    String userPromptTemplate;

    private static final String DEFAULT_SUMMARIZATION_SYSTEM_PROMPT = """
    ...
    private static final String DEFAULT_SUMMARIZATION_USER_PROMPT_TEMPLATE = """
            Entity: %s
            %s
    ...
    private CompletableFuture<String> performSummarization(
        ...
        return summarizationFuture.thenCompose(summary -> {
            ...
            return cacheStorage.store(
                    ...
                    contentHash,
                    summary,
                    TokenUtil.estimateTokens(summary)).thenApply(cacheId -> {
    ...
    private CompletableFuture<String> directSummarize(
        ...
        return llmFunction.apply(prompt, systemPrompt, null,
